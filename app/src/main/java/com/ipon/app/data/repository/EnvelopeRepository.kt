...
class EnvelopeRepository(
    ...
    suspend fun setCustomIcon(category: ExpenseCategory, periodYearMonth: String, icon: String?) {
        val existing = envelopeDao.getForCategoryAndPeriod(periodYearMonth, category.displayName)
        if (existing != null) {
            envelopeDao.upsert(existing.copy(customIcon = icon))
        }
    }

    suspend fun setCap(category: ExpenseCategory, periodYearMonth: String, cap: Money, customIcon: String? = null) {
        val existing = envelopeDao.getForCategoryAndPeriod(periodYearMonth, category.displayName)
