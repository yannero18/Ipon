 ...
 * arithmetic over thousands of transactions. A purchase of ₱150.50 is stored as 15050L.
 *
 * Design notes:
...
value class Money private constructor(val minorUnits: Long) : Comparable<Money> {
     ...
     * percentage). This is the one place fractional math can creep in, so it
     * is routed through BigDecimal with explicit HALF_EVEN (banker's rounding) per
    ...
    /** Renderers using PHP grouping/decimal conventions with tabular-friendly fixed decimals. */
    fun formatPhp(showSign: Boolean = false): String {
        return CurrencyFormatter.format(this, showSign)
    }
